...
def evaluate_hparams(hparams, dataset, y_tr, y_val, y_test, use_fuzzy=False, model_dir_hint=None):
    ...
    # Eval (publication-safe):
    # - model outputs logits -> convert to probabilities
    # - choose threshold on validation set to avoid test leakage
    probs_val = predict_proba(model, X_val)
    thr = select_threshold(probs_val, y_val, metric="f1")
