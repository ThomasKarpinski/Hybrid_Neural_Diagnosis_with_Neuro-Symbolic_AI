...
class EnhancedALSHADE:
    ...
    def run(self, X_train, y_train, X_val, y_val, input_dim):
        ...
        while self.eval_count < self.max_evals:
            ...
            for i in range(pop_size_curr):
                ...
                res = train_and_eval_on_val(hparams, X_train, y_train, X_val, y_val, input_dim, seed=seed_eval, device=self.device)
    ...
    def _evaluate_ind(self, i, X_train, y_train, X_val, y_val, input_dim):
        ...
        res = train_and_eval_on_val(hparams, X_train, y_train, X_val, y_val, input_dim, seed=self.seed + self.eval_count, device=self.device)
        score = res["roc_auc"]
        if score is None or np.isnan(score):
            score = 0.0
...
def run_enhanced_alshade(
    ...
    optimizer = EnhancedALSHADE(bounds, pop_size, max_evals, seed, fixed_hparams=fixed_hparams, device=device)
