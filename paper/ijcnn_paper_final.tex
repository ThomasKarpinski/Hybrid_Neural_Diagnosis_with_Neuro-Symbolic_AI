...
\caption{Autoencoder Latent Space Visualization. The 10-dimensional bottleneck is reduced to 2D via PCA. Unlike the linear PCA on raw features, the AE embedding space shows a more complex, non-linear manifold structure, though class overlap remains a challenge.}
\label{fig:ae_latent}
\end{figure}
...
\textbf{Analysis:} As hypothesized, Bayesian Optimization (Optuna) proved the most efficient, rapidly identifying a high-performing configuration (ROC-AUC $\approx 0.81$) within fewer trials than Random Search. The Genetic Algorithm also performed well but required a larger number of evaluations to stabilize. The optimized MLP achieved a final Test ROC-AUC of \textbf{0.8106}, significantly robust for this complexity of data.
...
\caption{Confusion Matrix for the Baseline MLP. The matrix reveals the distribution of True Positives, False Positives, True Negatives, and False Negatives.}
...
\begin{itemize}
    ...
    \item \textbf{Correction of False Alarms:} Conversely, when the MLP produced a False Positive (incorrectly predicting diabetes), the Fuzzy Inference system correctly indicated "Low Risk" in \textbf{93.65\%} of cases.
...
\caption{Confusion Matrix for the Hybrid Neuro-Symbolic System. Compared to the baseline MLP, the hybrid system demonstrates improved sensitivity to the minority class.}
\label{fig:cm_hybrid}
\end{figure}
...
\caption{ROC Curve Comparison. The Hybrid System (solid line) shows superior discrimination capability compared to the standalone Baseline MLP.}
...
\begin{itemize}
    \item \textbf{The Safety--Performance Trade-off:} The pure MLP achieved a higher ROC-AUC (0.81) compared to the Hybrid system (0.71). However, AUC is an aggregate metric that treats all errors equally. In a diagnostic setting, a False Negative (missing a sick patient) is catastrophic. Our Hybrid system's rule-based layer successfully recovered 80.02\% of the cases that the MLP missed. The drop in AUC reflects the system's "conservative" nature—it refuses to classify high-risk profiles as healthy even if the statistical signal is weak, effectively flattening the ROC curve but securing the decision boundary for critical cases.
    \item \textbf{Winning Configuration:} Among the extensive combinations of representations, optimizers, and HPO strategies, the \textbf{PCA representation paired with the Lion optimizer and Bayesian Optimization (Optuna)} emerged as the superior statistical baseline. This combination achieved the highest test ROC-AUC of \textbf{0.8212}, demonstrating that a compressed, variance-maximizing feature space (PCA) allows modern adaptive optimizers like Lion to navigate the loss landscape more effectively than raw features or complex autoencoder embeddings.
...
\section{Conclusion}
 Our journey began with a question: Can we make a high-performance neural network safe enough for medicine? We started by rigorously optimizing a deep learning baseline, using advanced evolutionary algorithms to squeeze every drop of performance from the data, achieving a respectable ROC-AUC of 0.81. Yet, statistical power alone is not trustworthiness.

 When we exposed this "black box" to a Neuro-Symbolic safety layer, we uncovered a critical reality. While the hybrid integration lowered the aggregate discrimination score to 0.71, it achieved something far more valuable: it caught 80\% of the false negatives that the neural network had silently missed. This result redefines success for medical AI. It is not about being right 99\% of the time on average; it is about \textit{never} being wrong when it matters most.
